...
import AttendanceRecycleBinModal from './AttendanceRecycleBinModal';
import { 
  formatCurrency, 
  formatAmount,
  formatNumber, 
...
export function CalendarReportsView({ onOpenLoggingModal }) {
  ...
  return (
    <div className="space-y-6 pb-20">
                                  ...
                                  {Boolean(log.isSettled || log.settlementReceiptId) ? (
                                    <div className="flex items-center gap-1 ms-1">
                                      <span className="flex items-center gap-0.5 text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-1.5 py-0.5 rounded border border-emerald-200 dark:border-emerald-800" title="تسویه شده (قفل)">
                                        ...
                                        <span>{language === 'fa' ? 'تسویه' : 'Settled'}</span>
                                      </span>
                                      <button
                                        onClick={() => setEditingLog(log)}
                                        className="p-0.5 text-slate-400 hover:text-emerald-600 transition-colors"
                                        title={language === 'fa' ? 'مشاهده جزئیات (تسویه شده - قفل)' : 'View details (settled - locked)'}
                                      >
                            ...
                            <td className="px-4 py-3 text-center no-print whitespace-nowrap">
                              <div className="flex items-center justify-center gap-1">
                                {logSettled ? (
                                  <button
                                    onClick={() => setEditingLog(log)}
                                    className="p-1.5 hover:bg-emerald-50 dark:hover:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400 rounded-lg transition-colors flex items-center gap-1 text-xs font-semibold"
                                    title={language === 'fa' ? 'مشاهده جزئیات (تسویه شده - قفل)' : 'View details (settled - locked)'}
                                  >
                                    <Lock className="w-3.5 h-3.5" />
                                    <span className="text-[10px]">{language === 'fa' ? 'قفل' : 'Locked'}</span>
                                  </button>
                                ) : (
                                  <>
                                    <button
      ...
      {/* Attendance Recycle Bin Modal */}
      <AttendanceRecycleBinModal
