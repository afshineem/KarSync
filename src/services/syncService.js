...
export async function syncWithSupabase() {
  ...
  if (!config.supabaseUrl || !config.supabaseUrl.trim()) {
    throw new Error('آدرس پروژه سوپابیس (Project URL) تنظیم نشده است.');
  }
  if (!config.supabaseKey || !config.supabaseKey.trim()) {
    throw new Error('کلید Publishable Key تنظیم نشده است.');
  }

  if (!navigator.onLine) {
    throw new Error('دستگاه آفلاین است. برای همگام‌سازی ابری اتصال اینترنت نیاز است.');
  }
  ...
  if (localWorkers.length > 0) {
    ...
    const wRes = await fetch(`${cleanUrl}/rest/v1/workers`, {
      method: 'POST',
      headers: {
        ...headers,
    ...
    if (!wRes.ok) {
      throw new Error(`خطا در ذخیره کارگران در سوپابیس (HTTP ${wRes.status})`);
    }
  }

  // 3. Push attendance logs to Supabase (Upsert)
  if (localLogs.length > 0) {
    ...
    const lRes = await fetch(`${cleanUrl}/rest/v1/attendance_logs`, {
      method: 'POST',
      headers: {
        ...headers,
        'Prefer': 'resolution=merge-duplicates'
    ...
    if (!lRes.ok) {
      throw new Error(`خطا در ذخیره گزارش‌های کارکرد در سوپابیس (HTTP ${lRes.status})`);
    }
  }

  // 4. Pull all records from Supabase to get other devices' updates
  const fetchWorkersRes = await fetch(`${cleanUrl}/rest/v1/workers?select=*`, { headers });
  if (!fetchWorkersRes.ok) throw new Error('خطا در دریافت اطلاعات کارگران از سوپابیس');
  const serverWorkers = await fetchWorkersRes.json();

  const fetchLogsRes = await fetch(`${cleanUrl}/rest/v1/attendance_logs?select=*`, { headers });
  if (!fetchLogsRes.ok) throw new Error('خطا در دریافت گزارش‌ها از سوپابیس');
  const serverLogs = await fetchLogsRes.json();

  // 5. Update local IndexedDB in a safe transaction
  ...
  window.dispatchEvent(new CustomEvent('workshop-sync-complete', {
    detail: { serverTime: nowTime, workers: serverWorkers.length, logs: serverLogs.length }
  }));

  return {
    success: true,
    serverTime: nowTime,
    workersSynced: serverWorkers.length,
    logsSynced: serverLogs.length
  };
}
